...
import maindata from "../BIBLES/GermanSchlachter.json";
...
function ActiveChapter(props) {
  ...
  rosdily = props.bible.books[props.book_num].chapters.map(
    (chapter, index) => ({
      chapter_id: index,
      chapter_name: `${props.bible.books[props.book_num].name} ${chapter.chapter}`,
    }),
  );
...
class VerseMemorizeGE extends Component {
  constructor(props) {
    super(props);
    this.state = {
      bible: maindata,
      ...
      verseNumber: 0,
      value_main: "",
      verse: "Im Anfang schuf Gott den Himmel und die Erde.",
      bible_words: [],
...
export default VerseMemorizeGE;
